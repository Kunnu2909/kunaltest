...
matchOps.getMatchDetails = async (req, res) => {
  ...
  db.query(sql_match, (err, [result]) => {
    ...
    const sql_team1 = `select sq.name, sq.player_id from (select * from matches where match_id = ${req.params.match_id}) m inner join squad sq on m.team1_id = sq.team_id`;
    ...
    db.query(sql_team1, (err, results) => {
      ...
      const sql_team2 = `select sq.name, sq.player_id from (select * from matches where match_id = ${req.params.match_id}) m inner join squad sq on m.team2_id = sq.team_id`;
